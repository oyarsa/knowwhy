...
def parse_args():
    ...
    parser.add_argument(
        "--comet-data-folder",
        type=str,
        help="Folder with relevant COMET relations in specific format",
    )
...
def create_tupsep_model_input_with_k_comet_relations(
    row, comet_dict, k, diversity=False
):
    """
    Add k relations in the below format
...
def create_verbalized_model_input_with_k_comet_relations(
    row, comet_dict, k, diversity=False
):
    """
    Add k relations in the below format
    ...
    for relation in relations:
        rel_type = relation["rel_type"]
        rel_phrase = relation["rel_phrase"].lstrip().rstrip()
        verbalized_comet_relation = get_verbalized_comet_relation(
            original_sentence_for_question, rel_type, rel_phrase
        )
