...
def parse_args():
    ...
    parser.add_argument(
        "--model-arch", type=str, help="Model architecture to use", default="t5-base"
    )
    parser.add_argument(
        "--input-type",
        type=str,
        help="Specify type of input format; normal, needs-effects",
    )
    parser.add_argument(
        "--data-folder", type=str, help="Folder with all the data from the corpus"
    )
    parser.add_argument(
        "--knowledge-folder", type=str, help="Folder with COMET data dictionaries"
    )
    ...
    parser.add_argument(
        "--model-dir",
        type=str,
        help="Directory to save model information in or load it from (for testing)",
    )
    parser.add_argument(
        "--log-file",
        type=str,
        required=True,
        help="Log filepath to write training information to",
    )
    parser.add_argument(
        "--source-len", type=int, default=150, help="Length of input context"
    )
    parser.add_argument(
        "--answer-len", type=int, default=30, help="Desired length of answer"
    )
    parser.add_argument(
        "--patience", type=int, default=3, help="Early stopping criteria"
    )
    ...
    parser.add_argument(
        ...
        dest="hidden_test",
        action="store_true",
        help="Set true if testing on hidden test set",
    )
...
def finetune(train_dataset, val_dataset, model, tokenizer, device, args):
    ...
    val_params = {"batch_size": args.batch_size, "shuffle": False, "num_workers": 0}
...
def finetuned_model_inference(test_dataset, model, tokenizer, device, args):
    ...
    test_params = {"batch_size": args.batch_size, "shuffle": False, "num_workers": 0}
...
def main(args):
    logging.basicConfig(
        filename=args.log_file, level=logging.DEBUG, format="%(asctime)s %(message)s"
    )
    ...
    if device.type == "cuda":
        ...
        logging.info(
            "Using cpu; did you set CUDA_VISIBLE_DEVICES in order to use a gpu?"
        )

    if args.dataset_name == "tellmewhy":
        train_df, dev_df, test_df, hidden_test_df = data.load_tellmewhy(
            args.data_folder
        )
    ...
    if args.input_type == "normal":
        ...
        (
            train_comet_dict,
            val_comet_dict,
            test_full_comet_dict,
            test_annotated_comet_dict,
        ) = data.load_comet_relations(args.knowledge_folder)

        k = 3
        diversity = True

        train_df = train_df.apply(
            lambda row: data.create_tup_model_input_with_k_comet_relations(
                row, train_comet_dict, k, diversity
            ),
            axis=1,
        )
        dev_df = dev_df.apply(
            lambda row: data.create_tup_model_input_with_k_comet_relations(
                row, val_comet_dict, k, diversity
            ),
            axis=1,
        )
        ...
        test_df = test_df.apply(
            lambda row: data.create_tup_model_input_with_k_comet_relations(
                row, test_full_comet_dict, k, diversity
            ),
            axis=1,
        )
        hidden_test_df = hidden_test_df.apply(
            lambda row: data.create_tup_model_input_with_k_comet_relations(
                row, test_annotated_comet_dict, k, diversity
            ),
            axis=1,
        )
        ...
        train_df = train_df.apply(
            lambda row: data.create_tupsep_model_input_with_k_comet_relations(
                row, train_comet_dict, k, diversity
            ),
            axis=1,
        )
        ...
        train_df = train_df.apply(
            lambda row: data.create_verbalized_model_input_with_k_comet_relations(
                row, train_comet_dict, k, diversity
            ),
            axis=1,
        )
    ...
    if args.mode == "train":
        ...
        model = T5ForConditionalGeneration.from_pretrained(
            args.model_arch, return_dict=True
        )
        ...
        train_df.to_json(
            os.path.join(args.model_dir, "train.json"), orient="records", indent=4
        )
        dev_df.to_json(
            os.path.join(args.model_dir, "val.json"), orient="records", indent=4
        )
        ...
        predictions, targets = finetuned_model_inference(
            test_df, model, tokenizer, device, args
        )
        ...
        writer = csv.DictWriter(
            out_fp, fieldnames, lineterminator="\n", quoting=csv.QUOTE_ALL
        )
        writer.writeheader()
